...
func (r *SeriesRepository) FindByLibraryID(libraryID string) ([]model.Series, error) {
	rows, err := database.DB.Query(
		`SELECT id, library_id, title, path, thumbnail_path, description, status, authors, tags, is_bookmarked, created_at, updated_at 
	...
	for rows.Next() {
		...
		if err := rows.Scan(&s.ID, &s.LibraryID, &s.Title, &s.Path, &thumbnail, &s.Description, &s.Status, &s.Authors, &s.Tags, &s.IsBookmarked, &s.CreatedAt, &s.UpdatedAt); err != nil {
			return nil, err
		}
		if thumbnail.Valid {
...
func (r *SeriesRepository) FindByID(id string) (*model.Series, error) {
	...
	err := database.DB.QueryRow(
		`SELECT id, library_id, title, path, thumbnail_path, description, status, authors, tags, is_bookmarked, created_at, updated_at FROM series WHERE id = ?`,
		id,
	).Scan(&s.ID, &s.LibraryID, &s.Title, &s.Path, &thumbnail, &s.Description, &s.Status, &s.Authors, &s.Tags, &s.IsBookmarked, &s.CreatedAt, &s.UpdatedAt)
...
func (r *SeriesRepository) FindByPath(path string) (*model.Series, error) {
	...
	var thumbnail sql.NullString
	err := database.DB.QueryRow(
		`SELECT id, library_id, title, path, thumbnail_path, description, status, authors, tags, is_bookmarked, created_at, updated_at FROM series WHERE path = ?`,
		path,
	).Scan(&s.ID, &s.LibraryID, &s.Title, &s.Path, &thumbnail, &s.Description, &s.Status, &s.Authors, &s.Tags, &s.IsBookmarked, &s.CreatedAt, &s.UpdatedAt)
...
func (r *SeriesRepository) Update(series *model.Series) error {
	now := time.Now()
	_, err := database.DB.Exec(
		`UPDATE series SET title = ?, path = ?, thumbnail_path = ?, description = ?, status = ?, authors = ?, tags = ?, is_bookmarked = ?, updated_at = ? WHERE id = ?`,
		series.Title, series.Path, series.ThumbnailPath, series.Description, series.Status, series.Authors, series.Tags, series.IsBookmarked, now, series.ID,
	)
	return err
}
