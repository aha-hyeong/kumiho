...
func (r *ChapterRepository) FindByPath(db database.Queryer, path string) (*model.Chapter, error) {
	...
	var c model.Chapter
	err := db.QueryRow(
		`SELECT id, volume_id, title, chapter_number, path, page_count, total_bytes, total_positions, has_audio, created_at, updated_at FROM chapters WHERE path = ?`,
		path,
...
func (r *ChapterRepository) UpdateHasAudio(db database.Queryer, chapterID string, hasAudio bool) error {
	db = database.GetQueryer(db)
	_, err := db.Exec(`UPDATE chapters SET has_audio = ? WHERE id = ?`, hasAudio, chapterID)
	return err
}
